...
import {
  Users, Search, Pencil, Trash2, Plus, X,
  CheckCircle2, User, Hash, Building2, GraduationCap,
  Smartphone, Phone, AlertCircle, Filter
} from 'lucide-react';
...
function EditModal({ student, onClose, onSave }) {
  ...
  const sanitize = (name, value) => {
    ...
    if (name === 'student_contact' || name === 'parent_contact') return value.replace(/\D/g, '').slice(0, 10);
    return value;
  };
  ...
  const handleChange = (e) => {
    ...
    setForm(prev => ({ ...prev, [name]: name === 'semester' ? Number(clean) : clean }));
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const res = await apiFetch(`http://localhost:5000/api/admin/students/${student._id}`, {
        method: 'PUT',
    ...
    <div className="ams-modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="ams-modal">
        ...
        <div className="ams-modal-header">
          ...
          <button className="ams-modal-close" onClick={onClose}><X size={18} /></button>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="ams-modal-body">
            ...
            <div className="ams-modal-field">
              <label className="ams-modal-label"><User size={12} /> Full Name *</label>
              <input
                type="text" name="name"
                className={fieldClass('name')}
                value={form.name} onChange={handleChange}
                placeholder="e.g. Ravi Kumar" autoComplete="off"
              />
              {errors.name && <span style={{ fontSize: '0.72rem', color: '#ef4444', marginTop: '0.1rem' }}>{errors.name}</span>}
            </div>

            {/* User ID (read-only — identifier) */}
            <div className="ams-modal-field">
              <label className="ams-modal-label"><Hash size={12} /> User ID</label>
              <input
              ...
              />
              <span style={{ fontSize: '0.72rem', color: 'var(--color-text-muted)' }}>User ID cannot be changed</span>
            </div>

            {/* Department + Semester */}
            <div className="ams-modal-row">
              <div className="ams-modal-field">
                <label className="ams-modal-label"><Building2 size={12} /> Department *</label>
                <select name="department" className="ams-modal-input" value={form.department} onChange={handleChange}>
                  {DEPARTMENTS.filter(d => d !== 'All').map(d => <option key={d} value={d}>{d}</option>)}
                </select>
              </div>
              <div className="ams-modal-field">
                <label className="ams-modal-label"><GraduationCap size={12} /> Semester *</label>
                <select name="semester" className="ams-modal-input" value={form.semester} onChange={handleChange}>
            ...
            <div className="ams-modal-row">
              <div className="ams-modal-field">
                <label className="ams-modal-label"><Smartphone size={12} /> Student Contact</label>
                <input
                  type="tel" name="student_contact"
                ...
                />
                {errors.student_contact && <span style={{ fontSize: '0.72rem', color: '#ef4444' }}>{errors.student_contact}</span>}
              </div>
              <div className="ams-modal-field">
                <label className="ams-modal-label"><Phone size={12} /> Parent Contact</label>
                <input
                  type="tel" name="parent_contact"
                  ...
                  inputMode="numeric" maxLength={10}
                />
                {errors.parent_contact && <span style={{ fontSize: '0.72rem', color: '#ef4444' }}>{errors.parent_contact}</span>}
              </div>
            </div>
...
export default function AdminManageStudents() {
  ...
  useEffect(() => {
    apiFetch('http://localhost:5000/api/admin/students')
      .then(r => r.json())
  ...
  const statDept = students.filter(s => s.department === 'Computer Engineering').length;
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      await apiFetch(`http://localhost:5000/api/admin/students/${id}`, { method: 'DELETE' });
    ...
    <AdminLayout>
      ...
      <div className="ams-students-header">
        <div>
          ...
          <p className="ams-students-sub">View, edit, and remove enrolled students</p>
        </div>
        <button className="ams-add-new-btn" onClick={() => navigate('/admin/students/new')}>
      ...
      <div className="ams-students-stats">
        ...
        <div className="ams-stat-card emerald">
          <div className="ams-stat-value">{students.filter(s => s.department === 'Computer Engineering').length}</div>
          <div className="ams-stat-label">CSE Students</div>
        </div>
        <div className="ams-stat-card amber">
          <div className="ams-stat-value">{[...new Set(students.map(s => s.semester))].length}</div>
      ...
      <div className="ams-table-card">
        ...
        <div className="ams-toolbar">
          ...
          >
            {DEPARTMENTS.map(d => (
              <option key={d} value={d}>{d === 'All' ? 'All Departments' : DEPT_SHORT[d] || d}</option>
            ))}
          </select>
            ...
            <div key={student._id} className="ams-student-row">
              ...
              <div className="ams-student-identity">
                <div className="ams-student-avatar">
                  <img
                    src={`https://ui-avatars.com/api/?name=${encodeURIComponent(student.name)}&background=4f46e5&color=fff&bold=true`}
                    alt={student.name}
                  />
                </div>
                <div>
                  <div className="ams-student-name">{student.name}</div>
                  <div className="ams-student-uid">{student.user_id || student.enrollment_no}</div>
                </div>
              </div>

              {/* Department */}
              <span className="ams-dept-tag">{DEPT_SHORT[student.department] || student.department}</span>
